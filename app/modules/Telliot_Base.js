...
const Instruction = {
    ...
    VOICE_TRIGGER_STOP_CMD:     0x0009, //새로
    ...
    _processRequestData (command, data) {
        ...
        switch(command) {
            ...
            case 'led_change':
            {
                ...
                keys.forEach(key => {
                    const { r, g, b } = data[key];  // data[key]에서 r, g, b 값을 추출
                    led_color.push(r, g, b);  // r, g, b 값을 led_color 배열에 추가
                });

                this._writeDebug(Loglevel.DEBUG, 'data : ' + led_color.join(', '));
                this._request_telliot_command(Instruction.EXT_DEVICE_LED_CHANGE_CMD, led_color, led_color.length);
            }
                break;
